...
prompt.get(authProperties, function(err, result) {
    if (err) {
        ...
        // Create credentials from user input and pass on to getUsers()
        var username = '';
        ...
        mkdirp(__dirname + '/data-sets/output/', function (err) {
            if (err) {
                ...
                request({
                    method: 'GET',
                    uri: 'https://' + result.subdomain + '.zendesk.com/api/v2/users.json'
                },
                ...
                requestBuilder(username, result.password, 'https://' + result.subdomain + '.zendesk.com/api/v2/incremental/users.json?start_time=946684800', result.exportFile);
            }
