...
function getOrgs(credentials, subdomain, csvFile) {
    ...
    request.get('https://' + credentials + subdomain + '.zendesk.com/api/v2/users.json?page=' + page, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            var data = JSON.parse(body);

            underscore._.each(data.users, function(value) {
                users.push({"name":value.name, "id":value.id, "url":value.email});
                userCount++;
            });
            ...
            if (data.next_page !== null) {
                ...
                console.log("RESULTS - Showing " + userCount + " users");
                console.log("=====================================================\n\n");
                console.log(JSON.stringify(users));
                var csvContent = json2csv.convert(users);
